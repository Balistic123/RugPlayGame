...
import { O as OPENROUTER_API_KEY } from './auth-CA5qtupB.js';
import { d as db, e as coin, t as transaction, u as user, l as priceHistory } from './index4-C6Efthht.js';
...
`;
const QuestionValidationSchema = objectType({
  isValid: booleanType(),
  requiresWebSearch: booleanType(),
  reason: stringType(),
  suggestedResolutionDate: stringType()
});
const QuestionResolutionSchema = objectType({
...
async function getCoinData(coinSymbol) {
  try {
    ...
    }).from(coin).leftJoin(user, eq(coin.creatorId, user.id)).where(eq(coin.symbol, normalizedSymbol)).limit(1);
    if (!coinData) {
      return null;
    }
    const [priceStats] = await db.select({
      maxPrice: sql`MAX(CAST(${priceHistory.price} AS NUMERIC))`,
      minPrice: sql`MIN(CAST(${priceHistory.price} AS NUMERIC))`
    }).from(priceHistory).where(eq(priceHistory.coinId, coinData.id));
    ...
    }).from(transaction).innerJoin(user, eq(transaction.userId, user.id)).where(eq(transaction.coinId, coinData.id)).orderBy(desc(transaction.timestamp)).limit(10);
    return {
      ...coinData,
...
async function getMarketOverview() {
  try {
    const topCoins = await db.select({
      ...
      volume24h: coin.volume24h,
      change24h: coin.change24h
    }).from(coin).where(eq(coin.isListed, true)).orderBy(desc(coin.marketCap)).limit(10);
    const [marketStats] = await db.select({
      totalCoins: sql`COUNT(*)`,
      totalMarketCap: sql`SUM(CAST(${coin.marketCap} AS NUMERIC))`,
      totalVolume24h: sql`SUM(CAST(${coin.volume24h} AS NUMERIC))`
    }).from(coin).where(eq(coin.isListed, true));
    const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1e3);
    const recentActivity = await db.select({
      totalTrades: sql`COUNT(*)`,
      totalVolume: sql`SUM(CAST(${transaction.totalBaseCurrencyAmount} AS NUMERIC))`,
      uniqueTraders: sql`COUNT(DISTINCT ${transaction.userId})`
    }).from(transaction).where(gte(transaction.timestamp, twentyFourHoursAgo));
    return {
...
function extractCoinSymbols(text) {
  const coinPattern = /\*([A-Z]{2,10})(?![A-Z])/g;
  const matches = [...text.matchAll(coinPattern)];
  return [...new Set(matches.map((m) => m[1]))];
}
async function validateQuestion(question, description) {
...
NON-EXISTENT: ${nonExistentCoins.map((symbol) => `*${symbol}`).join(", ")} - Do not exist on platform`;
      ...
      if (existingCoins.length > 0) {
        coinContext += `
EXISTING: ${existingCoins.map(
          (coin2) => coin2 ? `*${coin2.symbol} (${coin2.name}): $${coin2.currentPrice.toFixed(6)}, Market Cap: $${coin2.marketCap.toFixed(2)}, Listed: ${coin2.isListed}` : "none"
        ).join("\n")}`;
      }
    }
  }
...
${marketOverview?.topCoins.slice(0, 5).map(
    (c) => `*${c.symbol}: $${c.currentPrice.toFixed(6)} (${c.change24h >= 0 ? "+" : ""}${c.change24h.toFixed(2)}%)`
...
- Provide a specific resolution date with time (suggest times between 12:00-20:00 UTC for good global coverage) The current date and time is ${(/* @__PURE__ */ new Date()).toISOString()}.
...
`;
  try {
    ...
    return {
      isValid: false,
      requiresWebSearch: false,
      reason: error instanceof Error && error.message.includes("rate limit") ? "AI service temporarily unavailable due to rate limits" : "Failed to validate question due to AI service error"
    };
  }
}
async function resolveQuestion(question, requiresWebSearch, customRugplayData) {
...
`;
  try {
    ...
    return {
      resolution: false,
      confidence: 0,
      reasoning: error instanceof Error && error.message.includes("rate limit") ? "AI service temporarily unavailable due to rate limits" : "Failed to resolve question due to AI service error"
    };
  }
}
async function getRugplayData(question) {
  try {
    const marketOverview = await getMarketOverview();
...
NON-EXISTENT COINS: ${nonExistentCoins.map((symbol) => `*${symbol}`).join(", ")} - These coins do not exist on the Rugplay platform`;
        ...
        if (existingCoins.length > 0) {
          coinSpecificData += `
EXISTING COINS DATA:
${existingCoins.map((coin2) => {
            if (!coin2) return "";
            return `
*${coin2.symbol} (${coin2.name}):
- Current Price: $${coin2.currentPrice.toFixed(8)}
- Peak Price: $${coin2.pricing.peak.toFixed(8)}
- Lowest Price: $${coin2.pricing.lowest.toFixed(8)}
- Market Cap: $${coin2.marketCap.toFixed(2)}
- 24h Change: ${coin2.change24h >= 0 ? "+" : ""}${coin2.change24h.toFixed(2)}%
- 24h Volume: $${coin2.volume24h.toFixed(2)}
- Pool: ${coin2.poolCoinAmount.toFixed(0)} ${coin2.symbol} + $${coin2.poolBaseCurrencyAmount.toFixed(2)} *BUSS
- Listed: ${coin2.isListed ? "Yes" : "No (Delisted)"}
- Creator: ${coin2.creatorName || "Unknown"} (@${coin2.creatorUsername || "unknown"})
- Created: ${coin2.createdAt.toISOString()}
- Recent trades: ${coin2.recentTrades.length} in last 10 transactions
${coin2.recentTrades.slice(0, 3).map(
              (trade) => `  ${trade.type}: ${trade.quantity.toFixed(2)} ${coin2.symbol} @ $${trade.pricePerCoin.toFixed(6)} by @${trade.username}`
            ).join("\n")}`;
          }).join("\n")}`;
        }
      }
    ...
    return `
Current Timestamp: ${(/* @__PURE__ */ new Date()).toISOString()}
...
${marketOverview?.topCoins.map(
      (coin2, index) => `${index + 1}. *${coin2.symbol} (${coin2.name}): $${coin2.currentPrice.toFixed(6)} | MC: $${coin2.marketCap.toFixed(2)} | 24h: ${coin2.change24h >= 0 ? "+" : ""}${coin2.change24h.toFixed(2)}%`
...
export { getRugplayData as g, resolveQuestion as r, validateQuestion as v };
//# sourceMappingURL=ai-Dt9WSCpP.js.map
