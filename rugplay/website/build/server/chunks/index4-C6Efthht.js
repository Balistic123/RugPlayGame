...
import { pgTable, integer, decimal, timestamp, boolean, varchar, text, serial, index, pgEnum, primaryKey, unique, check } from 'drizzle-orm/pg-core';
import { sql } from 'drizzle-orm';

const transactionTypeEnum = pgEnum("transaction_type", ["BUY", "SELL", "TRANSFER_IN", "TRANSFER_OUT"]);
const predictionMarketEnum = pgEnum("prediction_market_status", ["ACTIVE", "RESOLVED", "CANCELLED"]);
const notificationTypeEnum = pgEnum("notification_type", ["HOPIUM", "SYSTEM", "TRANSFER", "RUG_PULL"]);
...
const coin = pgTable("coin", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
  symbol: varchar("symbol", { length: 10 }).notNull().unique(),
  icon: text("icon"),
  // New field for coin icon
  creatorId: integer("creator_id").references(() => user.id, { onDelete: "set null" }),
  // Coin can exist even if creator is deleted
...
const promoCodeRedemption = pgTable("promo_code_redemption", {
  id: serial("id").primaryKey(),
...
const predictionQuestion = pgTable("prediction_question", {
  id: serial("id").primaryKey(),
  creatorId: integer("creator_id").references(() => user.id, { onDelete: "set null" }),
  ...
  // true = YES, false = NO, null = unresolved
...
const predictionBet = pgTable("prediction_bet", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => user.id, { onDelete: "set null" }),
...
const accountDeletionRequest = pgTable("account_deletion_request", {
  id: serial("id").primaryKey(),
  ...
  return {
    ...
    oneOpenRequest: index("account_deletion_request_open_idx").on(table.userId).where(sql`is_processed = false`)
  };
});
...
const client = globalForDb.client ?? postgres(private_env.DATABASE_URL);
if (private_env.NODE_ENV !== "production") globalForDb.client = client;
const db = drizzle(client, { schema });

export { predictionBet as a, accountDeletionRequest as b, comment as c, db as d, coin as e, account as f, promoCodeRedemption as g, userPortfolio as h, commentLike as i, promoCode as j, schema as k, priceHistory as l, notifications as n, predictionQuestion as p, session as s, transaction as t, user as u };
//# sourceMappingURL=index4-C6Efthht.js.map
