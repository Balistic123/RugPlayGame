...
import { G as get } from './index2-D4eROfHK.js';
...
function formatDateWithYear(timestamp) {
  const date = new Date(timestamp);
  return date.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
...
function formatRelativeTime(timestamp) {
  ...
  if (years < 1) {
    ...
    const remainingDays = Math.floor((now.getTime() - tempDate.getTime()) / (1e3 * 60 * 60 * 24));
...
function formatTimeUntil(date) {
  const now = Date.now();
  const target = typeof date === "number" ? date * (date < 1e12 ? 1e3 : 1) : new Date(date).getTime();
  let diff = Math.max(0, target - now);
  const minutes = Math.floor(diff / 6e4);
  const hours = Math.floor(diff / 36e5);
  const days = Math.floor(diff / 864e5);
  if (diff < 6e4) return "less than 1m";
  if (minutes < 60) return `${minutes}m`;
...
function showConfetti(confetti) {
...
function showSchoolPrideCannons(confetti) {
...
function timeToLocal(originalTime) {
  ...
  return Math.floor(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes(), d.getSeconds(), d.getMilliseconds()) / 1e3);
}
...
function calculateMinesMultiplier(picks, mines, betAmount) {
  ...
  const MAX_PAYOUT = 2e6;
  const HIGH_BET_THRESHOLD = 5e4;
  ...
  return Math.max(1, Number(effectiveMultiplier.toFixed(2)));
}

export { PRESTIGE_COSTS as P, getPrestigeCost as a, getPublicUrl as b, cn as c, getTimeBasedGreeting as d, calculateMinesMultiplier as e, formatValue as f, getPrestigeName as g, showSchoolPrideCannons as h, formatTimeUntil as i, formatDateWithYear as j, getPrestigeColor as k, formatQuantity as l, formatRelativeTime as m, formatPrice as n, formatMarketCap as o, playSound as p, PRESTIGE_NAMES as q, debounce as r, showConfetti as s, timeToLocal as t, formatDate as u };
//# sourceMappingURL=utils2-CLIgW4-x.js.map
